---
title: Linux计划任务以及进程检测与控制
date: 2023-11-27 12:25:00
categories:
- 学习
tags:
- 运维
...
案例：把普通账号reimu加入到`cron.deny`黑名单中，禁止其创建计划任务
...
第三步：把你需要禁止的用户名单，加入此文件（如reimu）

```powershell
reimu
```

切换到reimu账号，测试是否可以使用crontab命令
...
# ll /var/spool/cron
total 4
-rw-------. 1 reimu reimu  0 Mar 24 09:50 reimu
...
第二步：使用tail -f命令监控`/var/log/cron`日志程序
